...
Treat the file shipped by the device as the canonical template. The following is a reconstruction for drafting or comparison, not a reason to discard firmware-specific comments or new keys:
...
`Sending_Interval(0.02-10s)` accepts a documented range of 0.02–10 seconds; the example value includes the `s` suffix. `LED_State` accepts lowercase `on` or `off`.
