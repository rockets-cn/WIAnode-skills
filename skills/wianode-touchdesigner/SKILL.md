...
7. Verify after the final edit with `get_td_node_errors`. For visual output, inspect the relevant TOP with `get_top_image`. Re-read the changed node parameters and, when physical observation is unavailable, label it `待用户确认`.
