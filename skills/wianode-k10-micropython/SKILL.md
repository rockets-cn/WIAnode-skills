...
4. Build a sensor-only tracer bullet first: connect Wi-Fi, connect MQTT with the template's field-tested design (`umqtt.simple` with two connections: `k10i-*` subscribes to `topic_input`, `k10o-*` publishes and never subscribes), parse strict JSON with `ujson`, print credential-free diagnostics, and update the K10 screen with partial redraws only.
